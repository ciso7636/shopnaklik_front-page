...
$('body').on('click', '.loader-switch .has-switch', function () {
    $(this).parents('.loader-switch').toggleClass('hide-options');
});
$('body').on('click', '.fullwidth-switch .has-switch', function () {
    $(this).parents('.fullwidth-switch').toggleClass('hide-options');
});


/* SETTINGS */

var pageContainer = "#main-wrapper"; //typically no need to change this
var enablePreview = true; //set to off to disable previews

var editableItems = new Array();
editableItems['.frameCover'] = [];
editableItems['.fa'] = ['color', 'background-color', 'font-size'];
editableItems['h1'] = ['color', 'font-size', 'font-weight'];
editableItems['h2'] = ['color', 'font-size', 'font-weight'];
editableItems['h3'] = ['color', 'font-size', 'font-weight'];
editableItems['h4'] = ['color', 'font-size', 'font-weight'];
editableItems['h5'] = ['color', 'font-size', 'font-weight'];
editableItems['h6'] = ['color', 'font-size', 'font-weight'];
editableItems['p'] = ['color', 'font-size', 'font-weight'];
editableItems['label'] = ['color', 'font-size', 'font-weight'];
editableItems['.btn'] = ['border-radius', 'font-size'];
editableItems['img'] = ['border-radius'];
editableItems['a'] = ['color', 'font-size', 'font-weight'];
editableItems['.white-section'] = ['background-color'];
editableItems['.grey-section'] = ['background-color'];
editableItems['.header-menu-container'] = ['background-color'];
editableItems['.transparent-black-background-color'] = ['background-color'];
editableItems['.main-background-color'] = ['background-color'];
editableItems['.title-block-style-1 .title-block-container'] = ['background-color'];
editableItems['.mobile-slider'] = ['background-image'];
editableItems['.form-control'] = ['color', 'font-size', 'font-weight'];
...
var editableContent = ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'a', 'label', 'li'];
    ...
    for(x=0; x<=99; x++) {
        ...
        if( localStorage.getItem("blocksElement"+x) !== null && localStorage.getItem("blocksFrame"+x) !== null ) {
            ...
            if( x >= 1 && pageNames[x] != undefined) {
                ...
                newLI = $('<li><a class="plink" href="#'+pageNames[x]+'">'+pageNames[x]+'</a><span class="pageButtons"><a class="fileEdit" href=""><span class="fui-new"></span></a><a href="" class="fileClone"><span class="fa fa-clone"></span></a><a class="fileDel" href=""><span class="fui-cross"></span></a><a href="#" class="btn btn-xs btn-primary btn-embossed fileSave"><span class="fui-check"></span></a></span></li>');
            ...
            if( ui.item.find('.frameCover').size() == 0 ) {
                ...
                if( ui.item.find('iframe').size() > 0 ) {//iframe thumbnails
                    ...
                    ui.item.find('iframe').load(function(){
                        ...
                        //Graphicfort Fix
                        ui.item.height(theHeight+"px");
                        ui.item.find('iframe').height(theHeight+"px");
                        ui.item.find('.frameCover').height(theHeight+"px");
...
$(function(){
    ...
    $('#pageList').on("click", ".frameCover > .htmlBlock", function(){
        ...
        if ($(this).closest('li').height() < 400) {
            $(this).closest('li').css({height: '400px'});
        } else {
    ...
    $('#pageList').on("click", "li .editorButtons .editCancelButton", function(){
        ...
        //Graphicfort Fix
        $(this).closest('li').height($(this).closest('li').find('iframe').height());
    ...
    $('#previewModal').on('shown.bs.modal', function (e) {
        ...
        $('#pageList > ul:visible').each(function(){
            ...
            newInput = $('<input class="gfort-contents-preview" type="hidden" name="page" value="">');
    ...
    $('#exportModal').on('shown.bs.modal', function (e) {
        ...
        //Graphicfort Fix
        $('.gfort-contents-input').remove();
        $('.gfort-images-input').remove();
        $('#gfort-images').remove();

        var imageInput,
            imagesArray = '',
            imagesREG = /[^\/]+(\.jpg|\.jpeg|\.png|\.gif|\.bmp|\.ico)/gi;
        ...
        $('#pageList > ul').each(function(){
            ...
            newInput = $('<input class="gfort-contents-input" type="hidden" name="pages['+$('#pages li:eq('+($(this).index()+1)+') a:first').text()+']" value="">');

            $('#exportModal form').prepend(newInput);

            //Graphicfort Fix
            $('iframe#skeleton').contents().find('.gfort-swiper-slider').each(function () {
                $(this).removeAttr('id');
                $(this).removeAttr('style');
                $(this).find('.swiper-pagination').removeAttr('id');
                $(this).find('.swiper-button-next').removeAttr('id');
                $(this).find('.swiper-button-prev').removeAttr('id');
            });
            $('iframe#skeleton').contents().find('.subscribe-form-block form').each(function () {
                $(this).removeAttr('id');
            });
            $('iframe#skeleton').contents().find('.contact-form-block form').each(function () {
                $(this).removeAttr('id');
            });
            $('iframe#skeleton').contents().find('.bmi-form-block form').each(function () {
                $(this).removeAttr('id');
            });
            $('iframe#skeleton').contents().find('input[type="checkbox"]').each(function () {
                $(this).removeAttr('id');
                $(this).next().removeAttr('for');
            });
            $('iframe#skeleton').contents().find('select').each(function () {
                $(this).removeAttr('id');
            });
            $('iframe#skeleton').contents().find('.background-video-block video').each(function () {
                $(this).removeAttr('id');
            });
            $('iframe#skeleton').contents().find('.progress-block').each(function () {
                $(this).removeAttr('id');
            });
            $('iframe#skeleton').contents().find('.gmap-block').each(function () {
                $(this).removeAttr('id');
            });
            $('iframe#skeleton').contents().find('.events-table-block').each(function () {
                $(this).removeAttr('id');
            });
            $('iframe#skeleton').contents().find('.photostack-block').each(function () {
                $(this).removeAttr('id');
            });
            $('iframe#skeleton').contents().find('.instagram-feed-block').each(function () {
                $(this).removeAttr('id');
                $(this).html('');
            });
            $('iframe#skeleton').contents().find('.twitter-feed-block').each(function () {
                $(this).removeAttr('id');
                $(this).html('');
            });
            $('iframe#skeleton').contents().find('.flickr-feed-block').each(function () {
                $(this).removeAttr('id');
            });
            $('iframe#skeleton').contents().find('.twitter-slider').each(function () {
                $(this).removeAttr('id');
            });
            $('iframe#skeleton').contents().find('.filter-items-wrapper').each(function () {
                $(this).removeAttr('id');
            });
            $('iframe#skeleton').contents().find('.gfort-recaptcha').each(function () {
                $(this).removeAttr('id');
                $(this).html('');
            });
            $('iframe#skeleton').contents().find('.gfort-select-wrapper').each(function () {
                $(this).parent().append('<label>' + $(this).find('label').text() + '</label>');
                $(this).remove();
            });
            $('iframe#skeleton').contents().find('.date-picker').each(function () {
                $(this).removeClass('hasDatepicker');
            });
            $('iframe#skeleton').contents().find('form').each(function () {
                $(this).removeAttr('novalidate');
            });
            $('iframe#skeleton').contents().find('.background-portfolio-grid-container').each(function () {
                $(this).find('li').each(function () {
                    $(this).removeAttr('style');
                    var GridPLink = $('a', this);
                    var GridPLinkStyle = GridPLink.css('background-image').split(/\/elements\//).slice(-1).join("/").slice(0, -2).replace('url("', '');
                    GridPLink.append('<img src="' + GridPLinkStyle + '"/>');
                    GridPLink.removeAttr('style');
                });
            });

            newInput.val($('iframe#skeleton').contents().find('html').html()+"</html>" );

            imagesArray = newInput.val().match(imagesREG);
            imageInput = $('<input class="gfort-images-input" type="hidden" name="images[' + $('#pages li:eq(' + ($(this).index() + 1) + ') a:first').text() + ']" value="' + imagesArray + '">');
            $('#exportModal form').prepend(imageInput);

        });

        imagesArray = '';
        $('.gfort-images-input').each(function () {
            imagesArray = imagesArray + ',' + $(this).val();
        });
        imagesArray = imagesArray.substring(1);
        imagesArray = imagesArray.replace(/,/g, '/');

        $('#exportModal form').prepend('<input type="hidden" name="gfort_images" id="gfort-images" value="' + imagesArray + '" />');
    ...
    $('#exportProject').on('click', function() {
        ...
        if (!jQuery('#ExportTemplate').length) {
            jQuery('#export-builder-template').append('<input id="ExportTemplate" type="hidden" name="ExportTemplate" value="" />');
        }

        var get_elements = JSON.stringify(localStorage);

        jQuery("#ExportTemplate").val(get_elements);
        jQuery("#export-builder-template").submit();
        jQuery("#ExportTemplate").val('');

    });

    //Import Project File
    $('#importProject').on('change', function(e){

        var file = document.getElementById("importProject").files[0];

        if (file) {

            var ext = $('#importProject').val().split('.').pop().toLowerCase();
            if($.inArray(ext, ['graphicfort']) === -1) {
                $('#errorImpModal').modal('show');
                $('#importProject').val('');
            } else {

                $('#impExpModal').modal('hide');
                $('#impProjectFileModal').modal('show');

                $('#impProjectFile').on('click', function () {
            ...
            alert("File can not be loaded!");
        }
    });
    $('#impExpModal').on('shown.bs.modal', function () {
        $('#importProject').val('');
    })
    ...
    //Graphicfort Fix ( Duplicate Page )
    $('#pages').on('click', 'li.active .fileClone', function(e){

        e.preventDefault();

        var newPageElAttr = $(this).parents('li.active').find('a:first').attr('href');
        ...
        //create the page structure
        newPageList = $('<ul>' + $(newPageElAttr).html() + '</ul>');
        newPageList.css('display','block');
        newPageList.attr('id', 'page'+($('#pages li').size()-1) );
