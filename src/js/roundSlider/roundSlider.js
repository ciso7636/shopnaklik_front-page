...
function imgTimer() {
	var images = document.querySelectorAll('.product-figure img');

	removeClasslist('visible', images);

	state.imgNumber = state.imgNumber >= 10 ? 0 : state.imgNumber + 1;
	images[state.imgNumber].classList.add('visible');
}

function round_slider_tune(event) {
	if (event === 'auto') {
		document.querySelector('.round-slider .selection .item').classList.add('auto-rotate');
		state.interval = setInterval(imgTimer, 2000);
	} else {
